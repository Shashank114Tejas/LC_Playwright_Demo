...
test.describe("Test Case Title", () => {
    ...
    for (const data of dataset) {
        test("Purchase Flow with Store Pickup and Order Confirmation.", async ({
            ...
            const orderNo = await test.step("Get the order number after successful payment", async () => {
            ...
            const expectedItemsData = await test.step("Navigate to My Orders page and extract order details", async () => {
            ...
            beforeActionStatus = await test.step("Check status before confirmation", async () => {
            ...
            const actualItemsData = await test.step("Confirm order", async () => {
                ...
                return await confirmationPage.navigateToConfirmationPageAndPerformAction(entity_Id, "confirm");
            });
            ...
            await test.step("Check status after confirmation", async () => {
                ...
                await expect(afterActionStatus, `Order status is not as expected: ${afterActionStatus}`).toBe("Confirmed");
