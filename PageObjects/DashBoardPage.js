...
class DashboardPage {
  ...
  async isAccountFunctionalityWorkingFine() {
    if (this.accountLink.isEnabled()) {
      ...
      if (
        (await this.myAccountPageHeader.textContent()) === "Customer Login" ||
        (await this.myAccountPageHeader.textContent()) === "My Account"
      ) {
        return true;
      }
      return false;
  ...
  async NavigateToMyAccountPage() {
    ...
    if ((await this.myAccountPageHeader.textContent()) === "My Account") {
      console.log("user is on My Account page");
    }
  }
  ...
  async findAndAddProductToCartUsingSKU(SKU_Value) {
    ...
    if ((await this.allProductsList.count()) > 0) {
      ...
      try {
        ...
        if (successmsg != null) console.log();
      ...
      console.log();
      console.log(
        `-> Did not found any product with the SKU value ${SKU_Value}`
      );
    }
  }
}
